package com.njht.webyun.management.common.util;

import java.io.*;
import java.util.Calendar;
import java.util.GregorianCalendar;

public class ProcCmd {
	private String charsetName = "UTF-8";
	private String logFilePath = "c:/log";
	
	public String getLogFilePath() {
		return logFilePath;
	}

	public void setLogFilePath(String logFilePath) {
		this.logFilePath = logFilePath;
	}

	public String getCharsetName() {
		return charsetName;
	}

	public void setCharsetName(String charsetName) {
		this.charsetName = charsetName;
	}

	public boolean exec(String commandstr){
		try{
			final Process pr = Runtime.getRuntime().exec(commandstr);
			
			new Thread(new Runnable(){
				@Override
				public void run() {
					try(
                            BufferedReader br_in = new BufferedReader(new InputStreamReader(pr.getInputStream(),charsetName));
						){ 
						String buff = null;
						while ((buff = br_in.readLine()) != null) { 
							writeComLog("Process out :" + buff);
							try {
                                Thread.sleep(100); } catch(Exception e) {}
						} 
						br_in.close(); 
					} catch (IOException ioe) {
						System.out.println("Exception caught printing process output.");
						ioe.printStackTrace(); 
					} finally { 
						try { 
						} catch (Exception ex) {}
					} 
				} 
			}).start(); 
	
			new Thread(new Runnable(){
				@Override
				public void run() {
					try(
                            BufferedReader br_err = new BufferedReader(new InputStreamReader(pr.getErrorStream(), charsetName));
						){ 
						String buff = null;
						while ((buff = br_err.readLine()) != null) { 
							writeComLog("Process err :" + buff);
							try {
                                Thread.sleep(100); } catch(Exception e) {}
						} 
						br_err.close(); 
					} catch (IOException ioe) {
						System.out.println("Exception caught printing process error.");
						ioe.printStackTrace(); 
					} finally { 
						try { 
						} catch (Exception ex) {}
					} 
				} 
			}).start(); 
			
			//阻塞线程,等待命令执行完毕
			pr.waitFor();
			pr.destroy();
			return true;
		}catch(Exception e){
			e.printStackTrace();
			return false;
		}
	}
	
	/**
     * 记录日志内容
     * */
    public synchronized void writeComLog(String str) {
        Calendar c = GregorianCalendar.getInstance();
        String filename = logFilePath +"/"+ "databackup-"+c.get(Calendar.YEAR)+ fillZero(1+c.get(Calendar.MONTH)+"", 2) + ".log";
        try {
            File f = new File(filename);
            File logPath = new File(logFilePath);
            if(!logPath.exists()) {
            	logPath.mkdirs();
            }
            if(!f.exists()) 
            {
            	f.createNewFile();
            }
            if(f.exists()==true){
            	try(
                        BufferedWriter bufOut = new BufferedWriter(new FileWriter(f,true));
            		){
            		String datetime = "" + c.get(Calendar.YEAR) + "-"
            				+ fillZero(1+c.get(Calendar.MONTH)+"", 2) + "-"
            				+ fillZero(""+c.get(Calendar.DAY_OF_MONTH), 2) + " "
            				+ fillZero(""+c.get(Calendar.HOUR), 2) + ":"
            				+ fillZero(""+c.get(Calendar.MINUTE), 2) + ":"
            				+ fillZero(""+c.get(Calendar.SECOND), 2);
            		bufOut.write("["+datetime+"] "+str + "\n");
            		System.out.println("["+datetime+"] "+str);
            	}
            } else {
            	try(
                        BufferedWriter bufOut = new BufferedWriter(new FileWriter(f));
            		){
	                String datetime = "" + c.get(Calendar.YEAR) + "-"
	                		+ fillZero(1+c.get(Calendar.MONTH)+"", 2) + "-"
	                		+ fillZero(""+c.get(Calendar.DAY_OF_MONTH), 2) + " "
	                		+ fillZero(""+c.get(Calendar.HOUR), 2) + ":"
	                		+ fillZero(""+c.get(Calendar.MINUTE), 2) + ":"
	                		+ fillZero(""+c.get(Calendar.SECOND), 2);
	                bufOut.write("["+datetime+"] "+str + "\n");
	                System.out.println("["+datetime+"] "+str);
            	}
            }
        } catch(Exception e) {
            System.out.println("Error");
        }
    }
    
    /*右对齐左补零*/
    static String fillZero(String str, int len) {
        int tmp = str.length();
        int t;
        String str1 = str;
        if(tmp >= len) {
            return str1;
        }
        t = len - tmp;
        for(int i = 0; i < t; i++ ) {
            str1 = "0" + str1;
        }
        return str1;
     }
}
