 ...
 *
 * @author piesat 2015-9-1 18:05:56
 */
public class JobCompleteHelper {
	...
	private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
		// 清除redis 缓存
		String redisKey = handleCallbackParam.getRedisKey();
		RedisService redisService = XxlJobAdminConfig.getAdminConfig().getRedisService();
		if (!StringUtils.isEmpty(redisKey) && redisService.exists(redisKey)) {
			redisService.remove(redisKey);
			logger.info(">>>>>>redis 缓存清理成功,对应redis key = {}",redisKey);
		}
