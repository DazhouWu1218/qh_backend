 ...
 * @Description: 生态质量估算预处理
 */
@Service("cimissPreManyFilesToInputHandlerService")
@Slf4j
public class CimissPreManyFilesToInputHandlerService extends BaseProductHandlerService {
    ...
    public String addInputXmlParam(TriggerParam triggerParam, ProductParam productParam, InputXmlParamDTO inputXmlParam) {
        ...
        dyMap.forEach((s,s2) -> {
            ...
            if(Boolean.TRUE.equals(!isInputFileInfo(((String)s)) && StringUtils.isNotEmpty((String)s2))){
                String path = FileNameUtils.dealFilePath((String) s2, issue);
                log.info(path);
                otherMap.put(s,path);
            }
        });
