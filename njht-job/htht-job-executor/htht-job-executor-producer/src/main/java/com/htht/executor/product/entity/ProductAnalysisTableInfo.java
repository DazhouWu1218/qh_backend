...
public class ProductAnalysisTableInfo {
    ...
    public String generateReplaceDataSql() {
        ...
        if(minFieldValueNums == Integer.MAX_VALUE){
        	minFieldValueNums = 0;
        }
    ...
    public List<String> generateReplaceDataSql2() throws Exception{
        ...
        System.out.println("generateReplaceDataSql2===minFieldValueNums "+minFieldValueNums);
        for (int j = 0; j < minFieldValueNums; j++) {
        	StringBuilder valSql = new StringBuilder();
        	valSql.append("(");
            for (int f = 0; f < fields.size(); f++) {
            	valSql.append(fields.get(f).getSqlValue(j));
                if (f != fields.size() - 1) {
                	valSql.append(",");
                }
            }
            ...
            lst.add(replaceDataSql.toString()+valSql.toString());
        }
        return lst;
    }
