...
public class SftpUtils {
	...
	public SftpUtils(FtpEntity ftpDTO) {
		this.host = ftpDTO.getIpAddr();
		this.port = ftpDTO.getPort();
		this.username = ftpDTO.getUserName();
    ...
    public List<String> getDataFileList(String remotePath, String pattern){
    	List<String> filenames = new ArrayList<String>();
    	try{
    		 connect();
    		Vector v = listFiles(remotePath);
    		...
    		if (v.size() > 0){
    			...
    			while (it.hasNext()){
    				...
    				String filename = entry.getFilename();
    				SftpATTRS attrs = entry.getAttrs();
    				if (!attrs.isDir()&& p.matcher(filename).find()){
    					filenames.add(filename+";"+attrs.getSize());
    				}
    			}
    		}
    		if (logger.isInfoEnabled()){
    			logger.info("目录" + remotePath
    					+ "中共有" + v.size() + "个文件符合要求");
    		}
    	}catch (SftpException e){
    		e.printStackTrace();
    ...
    /**
     * 下载单个文件
     * @param forFilePath	原文件路径+原文件名
     * @param toFilePath	目标文件路径+目标文件名
     * @return
     */
    public boolean downloadFile(String forFilePath,String toFilePath)
    {
    	FileOutputStream fieloutput = null;
    	try
    	{
    		File forFile = new File(forFilePath); 
    		sftp.cd(forFile.getParent());
    		File file = new File(toFilePath);
    		mkdirs(toFilePath);
    		fieloutput = new FileOutputStream(file);
    		sftp.get(forFilePath, fieloutput);
    		if (logger.isInfoEnabled())
    		{
    			logger.info("===DownloadFile:" + forFile.getName() + " success from sftp.");
    		}
    		return true;
    	}
    	catch (FileNotFoundException e)
     ...
     * @return
     */
    public boolean uploadFile(String toFilePath,String forFilePath)
    {
    	FileInputStream in = null;
    	try
    	{
    		...
    		File file = new File(forFilePath);
    		in = new FileInputStream(file);
    		sftp.put(in, toFile.getName());
    		return true;
    	}
    	catch (FileNotFoundException e)
    	{
