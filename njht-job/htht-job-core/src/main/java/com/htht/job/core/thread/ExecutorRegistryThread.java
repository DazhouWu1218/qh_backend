...
import com.htht.job.core.util.SystemInfoUtil;
            ...
            public void run() {
                ...
                while (!toStop) {
                    try {
                        ...
                        // 获取当前执行器的内存以及cpu 使用情况（5s 内未获取结果则超时,超时之后不进行注册）
                        SystemInfoUtil.registryCpuAndMemoryInfo(registryParam);
