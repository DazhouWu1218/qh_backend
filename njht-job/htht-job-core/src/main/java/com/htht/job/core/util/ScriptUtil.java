...
public class ScriptUtil {
     ...
     * @throws IOException
     */
    public static int execCmd(String command, String param, String seconds) throws Exception {
        XxlJobHelper.log("开始执行 算法调度:[{}]", command);
        long timeout = 2 * 60 * 60L;
        if (StringUtils.isEmpty(seconds) || "0".equals(seconds)) {
            timeout = 24 * 60 * 60L;
        } else {
            try {
        ...
        assert jobLogFileName != null;
        try (FileOutputStream fileOutputStream = new FileOutputStream(jobLogFileName, true)) {
            ...
            XxlJobHelper.log("算法执行结束:[{}],耗时:{}ms", command, System.currentTimeMillis() - startMilliseconds);
            return exitValue;
        } catch (Exception e) {
            XxlJobHelper.log(e);
            throw e;
    ...
    public static void execute(String exePath, String inputXmlPath,String seconds) throws Exception {
        try {
            execCmd(exePath + " " + inputXmlPath, null, seconds);
        }catch (Exception e){
