...
public class FileUploadUtils {
     ...
     * @return
     */
    public static  ReturnT<List<String>> uploadFiles(List<MultipartFile> files,String upLoadPath) {
        List<String> list = new ArrayList<>();
        for(MultipartFile file:files){
            ReturnT<String> r = uploadFile(file, upLoadPath);
     ...
     * @return
     */
    public static ReturnT<String> uploadFile(MultipartFile file,String upLoadPath) {
        //获取原始名字
        ...
        // 上传之后的文件名 (命名格式为 用户名 + 当前时间)

        Integer userId = UserUtil.getCurrentUser().getUserId();
        filename = userId + DateFormatUtils.currentDateToStr(DateFormatUtils.currentTimeMillis()) +  filename.substring(filename.lastIndexOf("."));
        // 上传文件
