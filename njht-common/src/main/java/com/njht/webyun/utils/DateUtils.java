...
/**
 * @author ：tianjm
 * @date ：Created in 2019/12/10 10:45
 * @description：工具类-日期处理
 * @modified By：
 * @version: $
 */
public class DateUtils {
    ...
    public static String dateDiffHours(Date beginDate, Date endDate) {
        ...
        double l = (double) endDate.getTime() - beginDate.getTime();
        return String.format("%.2f", l/(1000*60*60));
    }
    ...
    /**
     * date类型进行格式化输出（返回类型：String  yyyy-MM-dd HH:mm:ss）
     * @param date
     * @return
     */
     ...
     * @return
     */
    public static Date getDayBeginByTime(Date date){
        Calendar cal=Calendar.getInstance();
        cal.setTime(date);
        cal.set(Calendar.HOUR_OF_DAY, 0);//0点
        cal.set(Calendar.MINUTE, 0);//0分
     ...
     * @return
     */
    public static Date getDayEndByTime(Date date){
        Calendar cal=Calendar.getInstance();
        cal.setTime(date);
        cal.set(Calendar.HOUR_OF_DAY, 23);//23点
        cal.set(Calendar.MINUTE, 59);//59分
